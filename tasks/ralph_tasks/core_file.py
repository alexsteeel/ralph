        ...
        if current_section is None:
            ...
            if metadata_match:
                ...
                if key == "status":
                    ...
                    if value:
                        task.depends_on = [
                            int(x.strip()) for x in value.split(",") if x.strip().isdigit()
                        ]
    ...
    if old_file and old_file.name != new_filename:
        ...
        if old_attachments_dir.exists() and old_attachments_dir.is_dir():
            try:
                ...
                logger.info(
                    f"Renamed attachments dir: {old_attachments_dir} -> {new_attachments_dir}"
                )
            except Exception:
...
def copy_attachment(
    project: str, task_number: int, source_path: str, filename: str | None = None
) -> Path:
    """
